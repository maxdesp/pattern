    ...
    def tags(self):
        ...
        for i, tag in enumerate(self.sentence.token):
            if tag == WORD:
                ...
                tags[i] = [
                    "-".join([str(x) for x in [ch.type] + list(reversed(r)) if x]) for r in ch.relations]
                tags[i] = "*".join(tags[i])
...
class Chunk(object):
    ...
    def modifiers(self):
        ...
        if self._modifiers is None:
            ...
            # VP-anchor.
            is_modifier = lambda ch: ch.type in (
                "ADJP", "ADVP") and ch.relation is None
...
class Sentence(object):
    ...
    def __init__(self, string="", token=[WORD, POS, CHUNK, PNP, REL, ANCHOR, LEMMA], language="en"):
        ...
        # Anchor tags related to anchor chunks or attached PNP's.
        self._anchors = {}
        # Helper variable: the last chunk's relation and role.
        self._relation = None
        # Helper variable: the last attachment tag (e.g., "P1") parsed in
        # _do_pnp().
        self._attachment = None
        # Helper variable: the last token parsed in parse_token().
        self._previous = None
    ...
    def append(self, word, lemma=None, type=None, chunk=None, role=None, relation=None, pnp=None, anchor=None, iob=None, custom={}):
        ...
        # Append Chunk, or add last word to last chunk.
        self._do_chunk(chunk, role, relation, iob)
    ...
    def parse_token(self, token, tags=[WORD, POS, CHUNK, PNP, REL, ANCHOR, LEMMA]):
        ...
        for k, v in izip(tags, token.split("/")):
            ...
            if k not in p:
                custom[k] = None
            # "type O negative" => "O" != OUTSIDE.
            if v != OUTSIDE or k == WORD or k == LEMMA:
        ...
        # Tokens may be tagged with multiple relations (e.g.,
        # NP-OBJ-1*NP-OBJ-3).
        if p[REL] is not None:
            ...
            # For PP relation tags (e.g., PP-CLR-1), the first chunk is PP, the
            # following chunks NP.
            if ch == "PP" \
                    and self._previous \
    ...
    def _do_anchor(self, anchor):
        ...
        if anchor:
            for x in anchor.split("-"):
                ...
                # attachment (PNP)
                if x.startswith("P") and len(self.pnp) > 0:
                    A, P = x.replace("P", "A"), x
    ...
    def slice(self, start, stop):
        ...
        for i, word in enumerate(self.words[start:stop]):
            ...
            # is by unpacking all of the token tags and passing them to
            # Sentence.append().
            # WORD
            p0 = word.string
            # LEMMA
            ...
            p2 = word.type
            # CHUNK
            p3 = word.chunk is not None and word.chunk.type or None
            # PNP
            p4 = word.pnp is not None and "PNP" or None
            p5 = word.chunk is not None and unzip(
                0, word.chunk.relations) or None  # REL
            p6 = word.chunk is not None and unzip(
                1, word.chunk.relations) or None  # ROLE
            # ANCHOR
            p7 = word.chunk and word.chunk.anchor_id or None
            p8 = word.chunk and word.chunk.start == start + \
                i and BEGIN or None      # IOB
            # User-defined tags.
            p9 = word.custom_tags
            # If the given range does not contain the chunk head, remove the
            # chunk tags.
            if word.chunk is not None and (word.chunk.stop > stop):
                p3, p4, p5, p6, p7, p8 = None, None, None, None, None, None
            # If the word starts the preposition, add the IOB B-prefix (i.e.,
            # B-PNP).
            ...
            # since it starts with the PP and ends with the chunk head (and is
            # meaningless without these).
            if word.pnp is not None and (word.pnp.start < start or word.chunk.stop > stop):
                p4, p7 = None, None
            s.append(word=p0, lemma=p1, type=p2, chunk=p3, pnp=p4,
                     relation=p5, role=p6, anchor=p7, iob=p8, custom=p9)
        s.parent = self
        s._start = start
        return s
...
# s.chunked(s)     => [Chunk('black cats/NP'), Chink('and/O'),
# Chunk('white dogs/NP')]
...
class Text(list):
    ...
    def xml(self):
        ...
        xml.append('<?xml version="1.0" encoding="%s"?>' %
                   XML_ENCODING.get(self.encoding, self.encoding))
...
# <sentence token="">, defines the order of tags in a token.
XML_TOKEN = "token"
...
def parse_xml(sentence, tab="\t", id=""):
    ...
    """
    uid = lambda * \
        parts: "".join([str(id), _UID_SEPARATOR] + [str(x) for x in parts]).lstrip(_UID_SEPARATOR)
    ...
    for word in sentence.words:
        ...
        if pnp and pnp.start == chunk.start:
            a = pnp.anchor and ' %s="%s"' % (
                XML_OF, uid("A", anchors.get(pnp.anchor.start, ""))) or ""
        ...
        if chunk and chunk.start == word.index:
            if chunk.relations:
                ...
                r1 = not len(unique(r1)) == 1 and "|".join(
                    r1) or (r1 + [None])[0]
                r2 = not len(unique(r2)) == 1 and "|".join(
                    r2) or (r2 + [None])[0]
            xml.append(indent + '<%s%s%s%s%s%s>' % (
                XML_CHUNK,
                chunk.type and ' %s="%s"' % (XML_TYPE, chunk.type) or "",
                chunk.relations and chunk.role != None and ' %s="%s"' % (
                    XML_RELATION, r2) or "",
                chunk.relation and chunk.type == "VP" and ' %s="%s"' % (
                    XML_ID, uid(chunk.relation)) or "",
                chunk.relation and chunk.type != "VP" and ' %s="%s"' % (
                    XML_OF, r1) or "",
                chunk.attachments and ' %s="%s"' % (
                    XML_ANCHOR, uid("A", anchors[chunk.start])) or ""
            ))
            indent = push(indent)
...
# {u'A1': [[u'eat', u'VBP', u'B-VP', 'O', u'VP-1', 'O', u'eat', 'O']]}
_anchors = {}
# {u'A1': [[[u'with', u'IN', u'B-PP', 'B-PNP', u'PP', 'O', u'with', 'O'],
_attachments = {}
...
def parse_string(xml):
    ...
    for sentence in dom(XML_SENTENCE):
        ...
        # so the format and order of the token tags is retained when
        # exporting/importing as XML.
        format = sentence.get(
        ...
        # The keys correspond to the keys in _attachments, which have linked
        # PNP chunks.
        if ANCHOR in format:
            ...
            for id in sorted(A.keys()):
                for token in A[id]:
                    token[i] += "-" + \
                        "-".join(["A" + str(a + p) for p in range(len(P[id]))])
...
def _parse_tokens(chunk, format=[WORD, POS, CHUNK, PNP, REL, ANCHOR, LEMMA]):
    ...
    if CHUNK in format:
        ...
        for j, token in enumerate(tokens):
            token[i] = token[i] != "O" and (
                (j == 0 and "B-" or "I-") + token[i]) or "O"
...
def _parse_token(word, chunk="O", pnp="O", relation="O", anchor="O",
    ...
    for tag in format:
        if tag == WORD:
            ...
            # Custom tags when the parser has been extended, see also
            # Word.custom_tags{}.
...
def nltk_tree(sentence):
    ...
    def do_pnp(pnp):
        # Returns the PNPChunk (and the contained Chunk objects) in NLTK
        # bracket format.
...
def graphviz_dot(sentence, font="Arial", colors=BLUE):
    ...
    for i, ch in enumerate(sentence.pnp):
        if ch.anchor:
            s += '\tpnp%s -> chunk%s;\n' % (i + 1,
                                            sentence.chunks.index(ch.anchor) + 1)
...
def table(sentence, fill=1, placeholder="-"):
    ...
    def format(token, tag):
        ...
        if tag == WORD:
            ...
            s = token.chunk and (
                token.index == token.chunk.start and "B" or "I")
        elif tag == CHUNK:
            s = token.chunk and token.chunk.type
        elif tag == ROLE:
            s = token.chunk and token.chunk.role
        elif tag == REL:
            s = token.chunk and token.chunk.relation and str(
                token.chunk.relation)
        elif tag == PNP:
    ...
    def outline(column, fill=1, padding=3, align="left"):
        # Add spaces to each string in the column so they line out to the
        # highest width.
    ...
    columns[3] = [columns[3][i] +
                  (iob == "I" and " ^" or "") for i, iob in enumerate(columns[2])]
    del columns[2]
    ...
    # and has extra spacing so that words across sentences line out nicely
    # below each other.
    for i, column in enumerate(columns):
        columns[i] = outline(
            column, fill + 10 * (i == 0), align=("left", "right")[i == 0])
